...
- **`gh pr create` fork-detection caveat:** when a repo is (or was) a fork, `gh` can resolve the PR base to the parent instead of the intended repo. Workaround: pass `--repo <owner>/<repo>` explicitly, or `gh api repos/<owner>/<repo>/pulls -f head=... -f base=...`.
